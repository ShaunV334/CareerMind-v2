...
export default function QuestionBank() {
  const { fetchQuestions, fetchCategories, loading, error } = useQuestions();
            ...
            {showAnswer && (
              <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-lg space-y-4">
                <div>
                  ...
                  <p className="text-sm">{selectedQuestion.explanation || "No explanation available"}</p>
                </div>
                <div>
                  <h3 className="font-semibold mb-2">Solution:</h3>
                  <pre className="bg-slate-900 text-slate-100 p-4 rounded overflow-x-auto text-xs">
                    {selectedQuestion.correctAnswer || "No solution available"}
                  </pre>
                </div>
              </div>
            )}
