...
aptitude.get('/questions', async (c) => {
  try {
    ...
    const filter: any = {};
    if (subcategoryId) filter.subcategoryId = new ObjectId(subcategoryId);
...
aptitude.post('/practice/:sessionId/submit-answer', async (c) => {
  try {
    ...
    if (isCorrect) {
      await db.collection('aptitude_practice_sessions').updateOne(
        { _id: new ObjectId(sessionId) },
        { $inc: { questionsCorrect: 1 } }
      );
    }
...
aptitude.post('/practice/:sessionId/complete', async (c) => {
  try {
    ...
    const result = await db.collection('aptitude_practice_sessions').updateOne(
      { _id: new ObjectId(sessionId) },
      {
        $set: {
...
aptitude.post('/bookmarks/:questionId', async (c) => {
  try {
    ...
    await db.collection('user_bookmarked_aptitude_questions').updateOne(
      { userId, questionId: new ObjectId(questionId) },
      {
        $set: {
          userId,
          questionId: new ObjectId(questionId),
