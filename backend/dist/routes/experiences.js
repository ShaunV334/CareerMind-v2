...
experiences.post("/", async (c) => {
    try {
        ...
        const { companyId, roleId, interviewDate, interviewRound, interviewType, durationMinutes, difficultyRating, experienceRating, outcome, feedback, questionsAsked } = await c.req.json();
...
experiences.put("/:experienceId", async (c) => {
    try {
        ...
        await db.collection("interview_experiences").updateOne({ _id: new ObjectId(experienceId), userId }, {
            $set: {
                feedback,
                outcome,
...
experiences.get("/preparation-plans/:planId/tasks", async (c) => {
    try {
        ...
        const filter = { prepPlanId: new ObjectId(planId), userId };
        if (completed)
...
experiences.post("/preparation-plans/:planId/tasks/:taskId/complete", async (c) => {
    try {
        ...
        await db.collection("interview_daily_tasks").updateOne({ _id: new ObjectId(taskId), userId }, {
            $set: {
                isCompleted: true,
...
experiences.post("/mock-interviews/:sessionId/complete", async (c) => {
    try {
        ...
        await db.collection("mock_interview_sessions").updateOne({ _id: new ObjectId(sessionId) }, {
            $set: {
