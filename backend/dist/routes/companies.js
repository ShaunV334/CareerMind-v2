...
companies.get("/:companyId/questions", async (c) => {
    try {
        ...
        if (roleId)
            filter.companyRoleId = new ObjectId(roleId);
        if (difficulty)
            filter.avgDifficulty = difficulty;
...
companies.post("/:companyId/sessions/:sessionId/complete", async (c) => {
    try {
        ...
        await db.collection("company_practice_sessions").updateOne({ _id: new ObjectId(sessionId) }, {
            $set: {
